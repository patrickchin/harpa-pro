...
- **2026-07-29** — The first `api-dev` deploy after PR #205 stopped before lifecycle arming when `storage-worker=1` tried to collapse Fly's active/standby pair; #210 auto-confirmed it and destroyed the active worker. Fix: remove explicit worker scaling, preserve deploy-then-arm, and forbid future pair reduction. [detail](2026-07-29-fly-worker-scale-confirmation.md)
