...
- **2026-06-06** *(R9)* — Placing a photo group into an issue/section card "worked" for a split second then reverted to the bottom Unplaced grid. `updateNotePlacement` was calling `bumpNotesChangedAt`, which caused `useAutoRegenerate` to fire a fresh LLM regen on every placement edit; the regen reshaped `issues[]`/`sections[]`, `splitPlacements` dropped the just-placed group into `orphans`, and `ReportTabPane`'s orphan-healer fire-and-forgot a `placement=null` PATCH. Fix: drop `bumpNotesChangedAt` from the placement service (placement is a UI-only annotation, not a content change); drop `'report'` from `useUpdateNotePlacementMutation` invalidation; flip the integration test to assert `notes_changed_at` does NOT bump; document the carve-out in `design-photo-placement.md`. [detail](2026-06-06-photo-placement-reverts-after-bump-regen-loop.md)
