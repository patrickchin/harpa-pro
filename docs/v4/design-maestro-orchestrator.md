...
| `check-no-maestro-point-taps.sh` | Greps `.maestro/**/*.yaml` / `.yml` for `point:` keys and rejects every coordinate except the exact, counted Xcode 27 native-Modal fallbacks in the two shared dialog helpers. Normal flows still require text, accessibility labels, or testIDs.                                                     | root `package.json` → `lint` script (chained via `&&`); self-tested by `scripts/ci/__tests__/check-no-maestro-point-taps.test.sh` | **keep standalone** — pure lint. `mo run` should inherit the same rule before launching device flows.                            |
