...
- **2026-07-29** — The first `api-dev` deploy after PR #205 applied migration 0022 and started the storage worker, then failed before lifecycle arming because Fly treated `storage-worker=1` as deleting its generated standby and required noninteractive confirmation. Fix: pass `--yes` in dev/prod and make the policy guard enumerate every storage-worker scale command. [detail](2026-07-29-fly-worker-scale-confirmation.md)
