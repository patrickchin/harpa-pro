...
- **2026-07-29** — PR-time Android smoke first stayed on Expo Dev Launcher's Home screen, then missed the first-run `Continue` modal when a cold bundle finished after its fixed probe. Fix: conditionally select the green `http://10.0.2.2:8081` row, wait fail-closed for `Continue|Email`, and retain hidden Maestro UI diagnostics. [detail](2026-07-29-expo-dev-launcher-server-not-selected.md)
