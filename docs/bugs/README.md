...
- **2026-07-29** — PR-time Android smoke stayed on Expo Dev Launcher's Home screen even though Metro was advertised as a green `http://10.0.2.2:8081` server, so the app UI never loaded. Fix: conditionally select that server row after the deep link and retain hidden Maestro UI diagnostics. [detail](2026-07-29-expo-dev-launcher-server-not-selected.md)
