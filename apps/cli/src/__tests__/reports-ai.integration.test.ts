...
beforeAll(async () => {
  ...
  await authOtpStart({ apiUrl: 'http://localhost', fetch: appFetch, email: 'cli-tests-reports-ai@e2e.harpapro.com', stdout: sink, stderr: sink });
  const code = await readLatestOtp('cli-tests-reports-ai@e2e.harpapro.com');
  const out = new MemoryStream();
  await authOtpVerify({
    apiUrl: 'http://localhost',
    fetch: appFetch,
    email: 'cli-tests-reports-ai@e2e.harpapro.com',
    code,
    raw: true,
    stdout: out,
    stderr: sink,
