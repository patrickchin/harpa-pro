...
export interface ReportPhotosProps {
  ...
  /**
   * When provided, photo groups are filtered to those with `placement === null`
...
export function ReportPhotos({ noteRows, onOpenPhoto, onOpenPlacementSheet }: ReportPhotosProps) {
  const groups = useMemo((): PhotoGroup[] => {
    ...
    for (const n of sorted) {
      // When placement UI is enabled, only show unplaced groups here —
      // placed groups render inline under their target issue/section.
      if (onOpenPlacementSheet && (n.placement ?? null) !== null) {
        continue;
      }
              ...
              {onOpenPlacementSheet ? (
                <View className="mb-2">
                  <PhotoPlacementChip
                    ...
                    testID={`btn-place-photos-${group.noteId}`}
                  />
                </View>
              ) : null}
