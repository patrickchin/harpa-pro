...
test("presents a static procurement journey and agent profiles", async ({
  page,
}) => {
  ...
  await expect(profiles.nth(0)).toContainText(
    "Architecture and material procurement",
  );
...
test("links the agents page from shared navigation without mobile overflow", async ({
  page,
}) => {
  await page.goto("/agents");
  ...
  await expect(
    mobileMenu.getByRole("link", { name: "Agents", exact: true }),
  ).toHaveAttribute("href", "/agents");
