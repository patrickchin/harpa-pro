...
**Fix.** Keep generated role counts as `string | null` through `GeneratedSiteReport`, preserve the raw count text in the mobile adapter, parse only for aggregate math and bar widths, and fall back to the qualitative phrase in report stats, worker cards, and HTML/PDF export.

**Test.** Added coverage in `generated-report.test.ts`, `report-body-adapter.test.ts`, `WorkersCard.test.tsx`, `report-ui.test.ts`, and `report-to-html.test.ts` for the `"a few"` count path.
