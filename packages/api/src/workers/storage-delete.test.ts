...
describe('storage delete worker', () => {
  it('stays idle when background maintenance is disabled', async () => {
    ...
    const warn = vi.fn();
    const error = vi.fn();
    const startStorageWorkerMemorySampling = vi.fn(() => vi.fn());
    const storageWorkerMemorySample = vi.fn();
    ...
    expect(startStorageWorkerMemorySampling).not.toHaveBeenCalled();
    expect(storageWorkerMemorySample).not.toHaveBeenCalled();
