...
- **2026-06-06** — Voice-recorder waveform was visually flat on **both** iOS and Android after the HARPA-PRO-D fix (#134). That fix moved every option out of the `new AudioModule.AudioRecorder(...)` constructor into `prepareToRecordAsync(...)` because the JS shim's `createRecordingOptions()` only flattens the nested `{ android: { audioEncoder: 'aac' } }` block in the prepare path. But `isMeteringEnabled` is captured at **constructor time** on both platforms — iOS via `AVAudioRecorder.meteringEnabled = YES` at init, Android via `AudioRecorder.kt:41 private var meteringEnabled = options.isMeteringEnabled` (and `prepareRecording()` builds a new `MediaRecorder` but never reads `isMeteringEnabled` again). With an empty constructor, `meteringEnabled` was false on Android and `meteringEnabled` was NO on iOS, so `recorder.getStatus().metering` came back `undefined`, our `amplitude` defaulted to 0, and all 30 waveform bars rendered at min height (audio file itself was unaffected — still AAC m4a, audible on playback). Fix: pass the *same* options object to **both** the constructor (so metering takes effect on both platforms) and `prepareToRecordAsync()` (so the shim still flattens the platform blocks for the AAC encoder). Companion to the AMR-NB fix below.
- **2026-06-06** — Voice-recorder waveform bars looked stuttery / 'wobble in place' after the cap-UI work (PR #134 commit `318807f9`). That commit scope-crept into rewriting `Waveform`'s plain `<View>` bars as a Reanimated `WaveformBar` whose `useEffect([targetHeight])` fires `withSpring` on every re-render. `useInlineRecorder` pushes a new amplitude sample every ~200 ms, which shifts every bar's value left by one slot — and with `key={idx}` (positional), every bar's spring re-fires from its previous value. Result: `HISTORY_SIZE` × overlapping mid-flight springs constantly being interrupted. Fix: revert `Waveform` to plain `<View>`s (the re-render *is* the animation at 5 Hz updates — what WhatsApp/Telegram do), drop the `react-native-reanimated` import, and add a header comment forbidding spring/timing wrappers for the same reason. Lesson: don't rewrite working animation primitives as a side-quest in unrelated PRs.
