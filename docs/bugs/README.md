...
- **2026-06-06** — Voice-recorder waveform animation looked broken/stuttery after the cap-UI work (#134). The scope-creep refactor wrapped each bar in a Reanimated `WaveformBar` that kicked off `withSpring` on `useEffect([targetHeight])`, so every amplitude sample (~10 Hz) started `HISTORY_SIZE` concurrent spring animations whose mid-flight overrides plus the parent re-render produced visible stutter. The original was a plain `<View>` whose height updates direct from re-render — that *is* the animation. Fix: revert `Waveform` to plain views, drop the `react-native-reanimated` import, and add a header comment forbidding spring/timing wrappers for the same reason. Lesson: don't rewrite working animation primitives as a side-quest in unrelated PRs.
