...
beforeAll(async () => {
  ...
  await authOtpStart({ apiUrl: 'http://localhost', fetch: appFetch, email: 'cli-tests-files@e2e.harpapro.com', stdout: sink, stderr: sink });
  const code = await readLatestOtp('cli-tests-files@e2e.harpapro.com');
  const out = new MemoryStream();
  await authOtpVerify({
    apiUrl: 'http://localhost',
    fetch: appFetch,
    email: 'cli-tests-files@e2e.harpapro.com',
    code,
    raw: true,
    stdout: out,
    stderr: sink,
...
describe('files presign + register + url', () => {
  it('presign mints an upload URL', async () => {
    ...
    expect(body.fileKey).toMatch(/^users\/usr_[0-9a-hjkmnp-tv-z]+\/scratch\/fil_[0-9a-hjkmnp-tv-z]+\.jpg$/);
