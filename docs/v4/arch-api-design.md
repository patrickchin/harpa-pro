...
| POST | `/api/auth/sign-in/email` | Test-account and App Review password sign-in (allow-listed via `TEST_ACCOUNT_EMAILS` / `APP_REVIEW_EMAILS`) |
...
`Retry-After` + `X-RateLimit-*` headers. App Review password sign-in
