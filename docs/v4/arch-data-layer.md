...
lib/api/
  ...
  errors.ts            # ApiError + response-envelope parsing
...
`apps/mobile/lib/api/hooks.ts` is generated by
`apps/mobile/scripts/gen-hooks.ts` when `pnpm gen:api` runs.

The generator keeps an explicit endpoint-to-hook table. It fails when
a mobile API operation has no entry or when an entry no longer exists
in the OpenAPI document. Public-site, admin, readiness, and operating-
system manifest routes are explicit exclusions.

The spec-drift check regenerates the OpenAPI types and hooks. It fails
when the committed output changes.
...
- `useOptimisticDeleteNote` — removes the cached row by id, rollback
  on error.
- `useOptimisticCreateReport` — inserts a pending draft into the
  report list and reconciles it with the server row.
...
`client.ts` maps non-2xx responses to `ApiError`. The object includes
`code`, `status`, `message`, and optional `requestId` and `details`.
Transport failures use `network_error`; invalid JSON success bodies
use `parse_error`. Callers inspect the code or use a feature-specific
mapper such as `usageLimitFromError()`.
...
The upload queue (`lib/uploads/queue.ts`) is the only persisted client
mutation worker. It stores user-scoped job snapshots in MMKV. After a
restart, it changes interrupted states to `pending` and repeats the
idempotent upload pipeline. Jobs whose local source file is gone are
dropped. See the Upload pipeline section in
[arch-mobile.md](arch-mobile.md#upload-pipeline).

## Auth header

`AuthSessionProvider` installs a token getter in `lib/api/auth.ts`.
The getter extracts the Better Auth session token from the cookie that
`@better-auth/expo` stores in SecureStore. `client.ts` awaits the getter
and attaches the bearer token to each request.

On 401, the client calls the installed unauthorized callback before it
throws. That callback clears uploads and query caches, signs out, and
lets the protected layout redirect to email sign-in.
...
- **`apps/mobile/lib/api/invalidation.test.ts`** — every
