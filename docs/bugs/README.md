...
- **2026-06-26** — Qualitative worker counts such as `"a few"` survived the API wire shape but disappeared in the rendered report because `report-core` / mobile adapters coerced role counts back to numbers and displayed `0`. Fix: keep role counts as `string | null`, parse only for math, and add report-card/stat/PDF/shared-view regressions. [detail](2026-06-26-qualitative-worker-count-hidden.md)
