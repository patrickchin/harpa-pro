 ...
 * live in `auth-schema.ts`, generated by `pnpm exec better-auth generate`
 * and committed as an artefact. Re-export them here so existing call
 * sites (`schema.users`, `schema.sessions`, etc.) keep working.
 *
 * IDs are bare `text`. The slug format (`usr_*`, `ses_*`, `vrf_*`,
 * `idn_*`) is enforced by better-auth's `advanced.database.generateId`
 * callback at write time — see `auth/auth.ts` and
