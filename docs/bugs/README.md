...
- **2026-07-29** — The first `api-dev` deploy after PR #205 stopped before lifecycle arming when `storage-worker=1` tried to collapse Fly's active/standby pair; #210 destroyed the active worker, and #212 correctly exposed the surviving stopped standby. Fix: remove broad scaling and repair only exact current-release singleton active or standby states, with a verified active/standby pair required before arming. [detail](2026-07-29-fly-worker-scale-confirmation.md)
