...
- **2026-07-29** — The first `api-dev` deploy after PR #205 stopped before lifecycle arming when `storage-worker=1` tried to collapse Fly's active/standby pair; #210 destroyed the active worker, #212 correctly exposed the survivor, and #213 showed Fly can leave it stopped after clearing standby. Fix: remove broad scaling, explicitly start only the exact stopped/no-standby candidate, and require fresh exact-candidate and final-pair proofs before arming. [detail](2026-07-29-fly-worker-scale-confirmation.md)
