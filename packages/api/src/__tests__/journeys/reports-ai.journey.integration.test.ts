...
describe('journey: report AI lifecycle', () => {
  it('generate → finalize → pdf signed url, then 409 on regenerate', async () => {
    ...
    const fin = await app.request(`/projects/${project.id}/reports/${report.number}/finalize`, { method: 'POST', headers: me.headers });
    ...
    const pdf = await app.request(`/projects/${project.id}/reports/${report.number}/pdf`, { method: 'POST', headers: me.headers });
