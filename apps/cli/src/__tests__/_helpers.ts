 ...
 * `packages/api/src/__tests__/journeys/_login.ts`. The `value` column
 * shape is `<otp>:<extra>`.
 *
 ...
 *   1. Wildcard injection — `email = '%@harpapro.com'` matches
 *      every test user's row at once.
 *   2. Substring oracle — `test@harpapro.com` matches a row whose
 *      identifier is `sign-in-otp-test2+test@harpapro.com.evil`,
