...
| 11d | **CLI TUI v4 (split-pane render)** | [arch-tui-layout.md](arch-tui-layout.md) | **OpenTUI + Solid on Bun: static two-pane layout (viewport + interaction), `Prompter` interface preserved, replaces clack rendering from 11a–c** — *layout/hierarchy superseded by 11e* |
| 11e | **CLI TUI v4.2 (layout redesign)** | [arch-tui-layout-v2.md](arch-tui-layout-v2.md) | **Three-row layout (TopBar/MainSplit/LogStrip), screen-taxonomy + per-breadcrumb viewport templates, kills "Pick an action" dominance and duplicated identity strip — supersedes 11d for layout** |
