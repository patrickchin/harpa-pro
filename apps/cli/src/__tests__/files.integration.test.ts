...
beforeAll(async () => {
  ...
  await admin.connect();
  await admin.query(
    `INSERT INTO "user"(id, name, email, email_verified, display_name, created_at, updated_at) VALUES ($1, 'CLI Files User', $2, true, 'CLI Files User', now(), now())`,
    [userId, 'files-user@example.com'],
  );
  await admin.end();
