...
describe('PhotoGroupPlacementSheet', () => {
  ...
  it('renders a Remove placement row only when a current placement exists', () => {
    ...
    expect(JSON.stringify(placedTree.toJSON())).toContain('btn-place-clear');
    ...
    expect(JSON.stringify(unplacedTree.toJSON())).not.toContain('btn-place-clear');
  });
  ...
  it('passes the chosen placement to onSelect', () => {
    ...
    const node = tree.root.findByProps({ testID: 'btn-place-section-1' });
    act(() => {
  ...
  it('passes null to onSelect when Remove placement is tapped', () => {
    ...
    const node = tree.root.findByProps({ testID: 'btn-place-clear' });
    act(() => {
      (node.props.onPress as () => void)();
    });
