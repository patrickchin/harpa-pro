 ...
 * (`"report"` wrapper, `"meta"`, `quantityUnit`, `sections`,
 ...
 *
 * Output shape is the unwrapped v4 `reportBody` — no `report`
 * wrapper, no `meta` envelope, no markdown fences.
 */
...
OUTPUT
Return ONLY valid minified JSON matching the SCHEMA below. The top-level value MUST be the report object itself — do NOT wrap it in a "report" envelope, do NOT include a "meta" field, do NOT wrap in markdown fences, do NOT add prose before or after.
...
{
  "visitDate":        ISO-8601 datetime string ("YYYY-MM-DDTHH:MM:SSZ") | null,
...
RULES
- "visitDate" — only set if the notes give an explicit date; otherwise null. Always emit a full ISO datetime (use T00:00:00Z if only a date is known).
...
EXAMPLE INPUT
EXISTING REPORT: {"visitDate":null,"weather":null,"workers":[],"materials":[{"name":"Concrete","quantity":50,"unit":"m³","status":"delivered","condition":null,"notes":null}],"issues":[],"nextSteps":["Cure for 24h"],"summarySections":[{"title":"Foundation Work","body":"Pour completed in zone A."}]}
NEW NOTES:
[1] Rebar delivery delayed to tomorrow morning.
EXAMPLE OUTPUT
{"visitDate":null,"weather":null,"workers":[],"materials":[{"name":"Concrete","quantity":50,"unit":"m³","status":"delivered","condition":null,"notes":null}],"issues":[{"title":"Rebar delivery delayed","severity":"medium","description":"Rebar delivery delayed to tomorrow morning.","action":null}],"nextSteps":["Cure for 24h","Follow up on rebar delivery"],"summarySections":[{"title":"Foundation Work","body":"Pour completed in zone A."}]}`;
