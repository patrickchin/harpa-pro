...
- Every Machine used by repair must match one unambiguous `app` identity on
  ...
  metadata remain exact. Untagged, digest-only, malformed, stale, transitional,
  or ambiguous initial inventories fail before mutation; no process-count
  scaling is allowed. Each update/start transition also proves the candidate
  id, singleton topology, empty services, and empty standbys. If clearing
  succeeds but later work fails, exact singleton stopped/no-standby and
  started/no-standby states are retry-safe; all other drift fails closed.
