...
sleep 0.5
assert_pass "succeeds when endpoint is already ready" \
  env READYZ_URL="http://127.0.0.1:$PORT/readyz" \
      READYZ_ATTEMPTS=2 READYZ_TIMEOUT=2 READYZ_SLEEP=1 \
      bash "$SCRIPT" >"$TMP/t1.log" 2>&1
kill "$SERVER_PID" 2>/dev/null || true
wait "$SERVER_PID" 2>/dev/null || true

# ---------------------------------------------------------------------------
# Test 2: Cold-start tolerated — server starts ~6s after deploy, well
# past the old 5s curl timeout. With the new defaults (timeout 30s,
# retries with sleep) the loop must succeed.
# ---------------------------------------------------------------------------
PORT="$(pick_port)"
SERVER_PID="$(start_delayed_server "$PORT" 6)"
assert_pass "succeeds against a 6-second cold-start (>5s curl)" \
  env READYZ_URL="http://127.0.0.1:$PORT/readyz" \
      READYZ_ATTEMPTS=4 READYZ_TIMEOUT=15 READYZ_SLEEP=2 \
      bash "$SCRIPT" >"$TMP/t2.log" 2>&1
kill "$SERVER_PID" 2>/dev/null || true
wait "$SERVER_PID" 2>/dev/null || true
...
# ---------------------------------------------------------------------------
PORT="$(pick_port)"
assert_fail "fails when endpoint never responds" \
  env READYZ_URL="http://127.0.0.1:$PORT/readyz" \
      READYZ_ATTEMPTS=2 READYZ_TIMEOUT=1 READYZ_SLEEP=1 \
      bash "$SCRIPT" >"$TMP/t3.log" 2>&1
grep -q "readyz never returned 2xx" "$TMP/t3.log" \
...
# ---------------------------------------------------------------------------
assert_fail "fails fast when READYZ_URL is unset" \
